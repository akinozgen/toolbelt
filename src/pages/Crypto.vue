...
const categories: CategoryDef[] = [
  {
    id: 'kdf', label: 'Key Derivation',
    items: [
      { id: 'bcrypt', label: 'Bcrypt',  component: lazy(() => import('./crypto/Bcrypt.vue')) },
      { id: 'argon2', label: 'Argon2',  component: lazy(() => import('./crypto/Argon2.vue')) },
      { id: 'pbkdf2', label: 'PBKDF2',  component: lazy(() => import('./crypto/Pbkdf2.vue')) },
    ],
  },
  {
    id: 'sym', label: 'Symmetric',
    items: [
      { id: 'hmac', label: 'HMAC',     component: lazy(() => import('./crypto/Hmac.vue')) },
      { id: 'aes',  label: 'AES-GCM',  component: lazy(() => import('./crypto/Aes.vue')) },
    ],
  },
  {
    id: 'misc', label: 'Misc',
    items: [
      { id: 'totp',     label: 'TOTP',              component: lazy(() => import('./crypto/Totp.vue')) },
      { id: 'strength', label: 'Password Strength', component: lazy(() => import('./crypto/Strength.vue')) },
    ],
  },
];

const DEFAULT_ITEM = 'bcrypt';
...
function selectItem(id: string) {
  ...
  if (route.query.item !== id) {
    router.replace({ path: '/crypto', query: { item: id } });
  }
}
...
<template>
  <div class="gen-page">
    <aside class="gen-sidebar">
      <div class="gen-sidebar-title">Crypto</div>
...
<style scoped>
/* Reuses .gen-page / .gen-sidebar / .gen-item / .gen-pane styles from Generate.vue scope.
   Since they're scoped, copy them here. */
