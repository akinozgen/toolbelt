...
async function run() {
  error.value = '';
  try {
    output.value = await text.unescape({ input: input.value, kind: kind.value });
...
<template>
  <div class="text-form">
    ...
    <div class="text-grid">
      <div class="text-pane">
        <label class="text-pane-label">Escaped input</label>
        <textarea v-model="input" class="text-area"></textarea>
      </div>
      <div class="text-pane">
        <label class="text-pane-label">Decoded</label>
